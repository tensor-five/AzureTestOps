...
Die Konfiguration trägt `version: 3`; Spalten speichern nur lokale ID, `versionSuiteId` und Sichtbarkeit. V4 ändert die gespeicherte Struktur nicht: Bestehende v3-Spalten, Filter, Zuordnungen und Gruppenzustände bleiben erhalten. Das historische Feld `catalogRootId` bleibt aus Kompatibilitätsgründen gespeichert, beeinflusst aber die Matrix nicht. Die Matching-Ansicht und ihre Stammsuite bleiben unverändert. Beim Umstieg aus v2 (oder einem älteren Stand) bleiben Suche, Filter, Set und Azure-Kontext erhalten. Alte tagbasierte Spalten, physische Tag-Mappings und freie Tag-Gruppen samt ihren Zuständen werden zurückgesetzt. Ein Hinweis fordert die einmalige Auswahl konkreter Versions-Suites und des neuen Gruppierungsmodus. `migratedFrom` hält die Herkunft über mehrfaches server- und browserseitiges Sanitizing fest; bereits konfigurierte v3-Einstellungen werden bei weiteren Neustarts nicht erneut zurückgesetzt. Explizite Zuordnungen verwenden das Tupel von Umgebung, Inhalt und Spalten-ID als Schlüssel und die physische Inhalts-Suite-ID als Wert.

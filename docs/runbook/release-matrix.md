...
Die verbindliche Funktionsreferenz ist der [freigegebene Vertrag v4](../contracts/release-matrix.v4.html) einschließlich seiner Verweise auf v3. Diese technische Übersicht erweitert ihn nicht.

## Bedienung

Im bestehenden Set zur „Release-Matrix“ wechseln. Unter „Spalten & Gruppierung“ pro Spalte eine konkrete Versions-Suite auswählen. Die sichtbaren gültigen Versionen liefern gemeinsam die Zeilenbasis; die frühere Stammsuite-Auswahl entfällt. Ihr aktueller vollständiger Titel ist die Überschrift; eine Umbenennung verliert die gespeicherte ID nicht. Pfad und ID unterscheiden gleichnamige Versions-Suites. Die Versionen dürfen an unterschiedlichen Stellen im selben Plan liegen. Der frühere Matrix-Stammkatalog hat keinen Einfluss mehr.

Die feste Hierarchie lautet Version → Umgebung → Inhalt → direkte Testfälle. Eine logische Zeile besteht aus Umgebungsname, Inhaltsname und Testfall-ID. Diese Kombination erscheint über alle sichtbaren ausgewählten Versionen einmal; abweichende Umgebungen oder Inhalte bleiben eigene Zeilen. Die Quelle wird ausschließlich unter der gewählten Version über exakt gleichnamige direkte Umgebungs- und Inhaltsknoten gesucht. Groß-/Kleinschreibung und der vollständige Name zählen. Suite-Tags, Query-Ausdrücke und Tag-Vererbung werden nicht verwendet.

Fehlende Quellpfade zeigen „?“, eine vorhandene Inhalts-Suite ohne direkte Testfall-Mitgliedschaft zeigt „·“. Mehrere gleiche direkte Pfade erfordern die konkrete Inhalts-Suite-Auswahl anhand Pfad/ID. Eine ungültige Auswahl wird nicht stillschweigend ersetzt. Ohne passendes Resultat bleibt der vorhandene Point-Outcome maßgeblich. Auch bei leerer Run-Liste werden etwa Passed, Failed oder Unspecified unverändert angezeigt. Fehlt ein Outcome, greift der bestehende NotRun-Fallback „—“.

Nur direkte Mitgliedschaften der Inhalts-Suites werden vereinigt. Fälle direkt an einer Versions-/Umgebungs-Suite oder unter weiteren Zwischenebenen liefern keine Zeilen. Ausblenden oder Entfernen einer Version entfernt deren exklusive Zeilen; ohne sichtbare gültige Version erscheint ein Auswahlhinweis. Ein nicht ausgewähltes Releasearchiv trägt keine Zeilen bei. Eine darin liegende Version kann ausdrücklich ausgewählt werden.
...
- UI: Hooks verwalten flüchtigen Ladezustand; ein nach Set, Plan und Kontext getrennter In-Memory-Store erhält laufende Schreibvorgänge und deren Rückmeldungen über Ansichtswechsel hinweg. Bestätigte Änderungen aktualisieren gezielt die betroffenen Statusprojektionen in beiden Ansichten. Darstellungsfunktionen bilden logische Zeilen aus dem Tupel von Umgebung, Inhalt und Testfall-ID. Die ausgewählte Ergebnisprojektion sowie Writes bleiben an die reale physische Suite und ihren Testpunkt gebunden.
...
Die Konfiguration trägt `version: 3`; Spalten speichern nur lokale ID, `versionSuiteId` und Sichtbarkeit. V4 ändert die gespeicherte Struktur nicht: Bestehende v3-Spalten, Filter, Zuordnungen und Gruppenzustände bleiben erhalten. Das historische Feld `catalogRootId` bleibt aus Kompatibilitätsgründen gespeichert, beeinflusst aber die Matrix nicht. Die Matching-Ansicht und ihre Stammsuite bleiben unverändert. Beim Umstieg aus v2 (oder einem älteren Stand) bleiben Suche, Filter, Set und Azure-Kontext erhalten. Alte tagbasierte Spalten, physische Tag-Mappings und freie Tag-Gruppen samt ihren Zuständen werden zurückgesetzt. Konkrete Versions-Suites müssen einmalig neu ausgewählt werden; der historische Migrationshinweis wurde auf Nutzerwunsch entfernt. `migratedFrom` hält die Herkunft über mehrfaches server- und browserseitiges Sanitizing fest; bereits konfigurierte v3-Einstellungen werden bei weiteren Neustarts nicht erneut zurückgesetzt. Explizite Zuordnungen verwenden das Tupel von Umgebung, Inhalt und Spalten-ID als Schlüssel und die physische Inhalts-Suite-ID als Wert.
...
Die 44 v4-Browserszenarien einschließlich Scrollregressionen starten echten lokalen Server, Adapter und Browseroberfläche mit isoliertem LowDB-Verzeichnis. Azure-HTTP wird ausschließlich im Test-Harness simuliert; es gibt keine Writes in einem Azure-Kundenprojekt. Die eingefrorenen v1/v2/v3-Szenarien bleiben historische Artefakte; ihre abgelöste Quellen- und Zeilenlogik ist nicht mehr die aktive Produktreferenz. `npm run test:e2e` verwendet die neue `playwright.v4.config.ts`; die ursprüngliche eingefrorene Konfiguration bleibt unverändert. Unit-Tests prüfen unter anderem vollständige Hierarchie, logische Zeilen, direkte Quellpfade, getrennte Gruppenzustände, Zielvalidierung, Aggregator-Wiederverwendung und idempotente Präferenzmigration. Testdateien und Verträge werden über eingecheckte Prüfsummen validiert.
...
Der Azure-Transport-Timeout (standardmäßig 60 Sekunden pro Aufruf) umfasst jetzt Authentifizierung, Fetch und Body. Timeout/Abbruch setzt ein echtes Fetch-AbortSignal; spät aufgelöste Authentifizierung darf danach keinen Request mehr starten. Navigation und ersetzte Browser-Reads brechen ihren Fetch ab; der Server beendet bei geschlossener Verbindung die Diagnose, bricht aktive Azure-GETs ab und verhindert weitere geplante Reads. Bereits laufende gemeinsame Azure-CLI-Authentifizierung wird nicht separat beendet. Retry-Backoff-Wartezeiten werden bei einem Abbruch sofort beendet; weitere Netzwerkaufrufe bleiben gesperrt. Es gibt keine neuen automatischen Write-Retries und keine zusätzliche globale Gesamtlaufzeitgrenze oder gekürzte Paging-/Run-Historie.

## Begrenzte Ansichtsabfragen

Die Matrix übergibt konfigurierte Versions-Suite-IDs als `versions`-JSON an ihren Read-Endpunkt. Nur die direkten Inhalts-Suites unter Version → Umgebung liefern Cases und Points. Ein leeres Array lädt ausschließlich die Hierarchie; fehlender Parameter erhält die bisherige vollständige API-Abfrage. Der Aggregator und die vollständige Ergebnishistorie bleiben unverändert, weil ältere physische Punkte weiterhin das neueste Suite-Ergebnis besitzen können. Run-/Result-Reads laufen parallel zur Work-Item-Hydration.

Der Browser speichert Snapshots maximal 60 Sekunden im Arbeitsspeicher, getrennt nach Client, Set, Plan, Root, Kontext und Versionsauswahl (maximal 16 Einträge). Explizites Aktualisieren umgeht den Cache. Unsichere Schreibvorgänge verhindern dessen Wiederverwendung; bestätigt ein späterer frischer Read den Write oder Reset, werden alle gecachten Versionsauswahlen desselben Set-/Plan-/Kontexts verworfen; ein gecachter Stand kann niemals einen unbestätigten Schreibvorgang freigeben. Das bestehende Mutationsjournal aktualisiert bestätigte Statuswerte auch beim Zurückwechseln.

Direkte Mitgliedschaften für externe Filter-Suites werden über `GET /phase2/sets/:id/release-matrix/memberships/:suiteId` gezielt geladen. Der vollständige Tagkatalog kommt erst bei Fokus der Tag-Auswahl über `GET /phase2/sets/:id/release-matrix/tags`. Beide Endpunkte prüfen den erfassten Set-/Plan-/Azure-Kontext; sie lesen keine Ergebnisse und besitzen abbrechbare, flüchtige UI-Caches. Tags benötigen planweite Mitgliedschaften und reine Tag-Hydration, ohne Points oder Historie. Ladefehler erscheinen als Toast.

Die Zuordnungsansicht verwendet pro SSE-Anfrage einen eigenen Leseclient. Verbindungsabbruch und ersetzte Anfragen brechen Azure-GETs und Retry-Wartezeiten ab; gestartete Worker werden vor Abschluss abgewickelt. Die Ausführungshistorie bleibt bei frischen Reads größenabhängig. Messwerte und Grenzen stehen im [Vergleichsbericht](../performance/view-read.md).

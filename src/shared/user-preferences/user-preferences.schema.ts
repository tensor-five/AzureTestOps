...
export type AdoContextPreference = {
  organization: string;
  project: string;
};
...
export function sanitizeUserPreferences(value: unknown): UserPreferences {
  ...
  const adoContext = sanitizeAdoContextPreference(candidate.adoContext);
  if (adoContext) {
    next.adoContext = adoContext;
  }
...
export function sanitizeAdoContextPreference(value: unknown): AdoContextPreference | null {
  if (!isPlainRecord(value)) {
    return null;
  }

  const organization = readNonEmptyString(value.organization);
  const project = readNonEmptyString(value.project);

...
import { HttpSetManagementAdapter } from "./http-set-management.adapter.js";
...
describe("HttpSetManagementAdapter", () => {
  let adapter: HttpSetManagementAdapter;

  beforeEach(() => {
    setCsrfMeta("csrf-abc");
    adapter = new HttpSetManagementAdapter();
  });
  ...
  it("normalizes the list response shape", async () => {
    installFetch(() =>
      jsonResponse(200, {
  ...
  it("URL-encodes set ids when targeting /phase2/sets/:id", async () => {
    ...
    await adapter.update("foo bar", { name: "x" });

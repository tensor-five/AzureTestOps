...
export type RuntimeOptions = {
  ...
  /** Override `~/.azure-testops/ado-context.json`. */
  adoContextFilePath?: string;
...
/**
 * Composition root for the local server. Wires lowdb persistence, ADO context
 * persistence, the auth preflight adapter, and a memoizing factory of Azure
 * adapters scoped to the currently configured ADO context.
 *
...
export function buildRuntime(options: RuntimeOptions = {}): Runtime {
  ...
  const adoContextFilePath =
    options.adoContextFilePath ?? path.join(os.homedir(), ADO_DIR_NAME, "ado-context.json");
  ...
  const adoContext = new FileAdoContextAdapter(adoContextFilePath);

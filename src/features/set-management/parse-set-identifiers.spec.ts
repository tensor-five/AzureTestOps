...
describe("parsePlanIdentifier", () => {
  it("returns the bare integer id unchanged", () => {
    expect(parsePlanIdentifier("42")).toBe("42");
    expect(parsePlanIdentifier("  42  ")).toBe("42");
  });
  ...
  it("extracts planId from a real Test Plans URL", () => {
    const url =
      "https://dev.azure.com/contoso/delivery/_testPlans/define?view=_TestManagement&planId=42&suiteId=43";
    expect(parsePlanIdentifier(url)).toBe("42");
  });

  it("ignores suiteId when asked for planId", () => {
    const url =
      "https://dev.azure.com/contoso/delivery/_testPlans/execute?suiteId=43&planId=42";
    expect(parsePlanIdentifier(url)).toBe("42");
  });
...
describe("parseSuiteIdentifier", () => {
  it("returns the bare integer id unchanged", () => {
    expect(parseSuiteIdentifier("43")).toBe("43");
  });

  it("extracts suiteId from a Test Plans URL", () => {
    const url =
      "https://dev.azure.com/contoso/delivery/_testPlans/define?view=_TestManagement&planId=42&suiteId=43";
    expect(parseSuiteIdentifier(url)).toBe("43");
  });
...
describe("parsePlanAndSuite", () => {
  it("extracts both ids from a Test Plans URL in one go", () => {
    const url =
      "https://dev.azure.com/contoso/delivery/_testPlans/define?view=_TestManagement&planId=42&suiteId=43";
    expect(parsePlanAndSuite(url)).toEqual({ planId: "42", rootSuiteId: "43" });
  });

  it("treats a bare integer as plan id only", () => {
    expect(parsePlanAndSuite("42")).toEqual({ planId: "42", rootSuiteId: null });
  });

  it("accepts manual id pairs separated by slash, comma or whitespace", () => {
    expect(parsePlanAndSuite("42/43")).toEqual({
      planId: "42",
      rootSuiteId: "43"
    });
    expect(parsePlanAndSuite("42, 43")).toEqual({
...
describe("parseQueryIdentifier", () => {
  ...
  it("extracts the GUID from a saved-query URL", () => {
    const url =
      "https://dev.azure.com/contoso/delivery/_queries/query/766fb375-befe-4752-add8-4b2d692f9c45/";

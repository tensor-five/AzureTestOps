...
type OutcomeAction = MatrixWrite['outcome'];
const outcomeActions: readonly OutcomeAction[] = [...manualOutcomes, 'ResetToActive'];
export type MatrixCellProps = {
    ...
    onConfigure(): void;
    onChange(outcome: OutcomeAction): void;
};
export function MatrixCell(props: MatrixCellProps) {
    const tooltip = useMatrixTooltip();
    const [keyboardOutcome, setKeyboardOutcome] = React.useState<OutcomeAction | null>(null);
    ...
    const label = p ? `${outcomeDisplay(p.lastOutcome).label} · ${p.title} · ${props.sourceDescription ?? p.suitePath}${reason ? ' · ' + reason : ''}` : reason;
    ...
    const preview = keyboardOutcome === 'ResetToActive' ? 'Unspecified' : keyboardOutcome;
    const display = p ? outcomeDisplay(preview ?? p.lastOutcome) : null;
    ...
    {p && display ? <><span aria-hidden="true" className={`relations-view-outcome-chip relations-view-outcome-chip-${display.slug}`}>{display.shortLabel}</span>
      <select aria-label={label} aria-describedby={tooltip.active?.id === id ? id : undefined} value={keyboardOutcome ?? p.lastOutcome} onKeyDown={e => {
            ...
            }} disabled={props.pending || props.pointCount !== 1 || !!props.readOnlyReason} onChange={e => { setKeyboardOutcome(null); props.onChange(e.target.value as OutcomeAction); }}>
        {!outcomeActions.includes(p.lastOutcome as OutcomeAction) && <option disabled value={p.lastOutcome}>{outcomeDisplay(p.lastOutcome).label}</option>}
        {outcomeActions.map(o => <option key={o} value={o}>{o === 'ResetToActive' ? 'Reset to active' : o}</option>)}

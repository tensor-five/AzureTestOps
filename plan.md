...
The tool is structurally and visually a sister project to AzureGanttOps — same Hexagonal architecture, same UI tokens (system font stack, primary `#842CC3`, secondary `#87F3A4`, header/footer shell, dark/light theme).

The repository is prepared for public open-source use with explicit license, contribution, conduct, security, and notice documents.
...
| 11 | Repo / OSS | **Open-source ready** under the Azure TestOps Attribution License | Public metadata and OSS governance files are present; final visibility is controlled in GitHub. |
...
Lives in **`~/.azure-testops/user-preferences.json`** as `adoContext: { organization, project }`. Reasons:
...
### 8.2 ADO context (user preferences)
...
```jsonc
{
  "users": {
    "<localUserId>": {
      "adoContext": { "organization": "<org>", "project": "<project>" }
    }
  }
}
```
...
- [x] `adapters/persistence/settings/lowdb-ado-context.adapter.{ts,spec.ts}` — reads/writes ADO context via `~/.azure-testops/user-preferences.json`
...
- [ ] Phase 5: ADO context bootstrap UX. If no `adoContext` is stored in `~/.azure-testops/user-preferences.json` on first run, the manage-sets dialog must collect org/project before it can let the user create a set.
...
- [x] OSS-readiness checklist (Phase 9): LICENSE, CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md, NOTICE.md, public README rewrite.

---

## 15 · Repo / Operations

- Remote: `https://github.com/tensor-five/AzureTestOps` (same org as AzureGanttOps). GitHub repository visibility is the remaining release switch.
- Default port `8081` (`PORT` env overrides) so it can run alongside AzureGanttOps on `8080`.
- Local data directory: `~/.azure-testops/` (`user-preferences.json`).

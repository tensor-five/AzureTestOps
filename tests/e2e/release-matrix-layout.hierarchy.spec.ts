...
test('V4-H01 RM4-01 RM4-03 selected versions union direct content rows independently of the retired catalog', async ({ page }) => {
    ...
    const expected = [
        ...
        ...[101,201].map(id => ['ACC','Regression',id]),
        ['ACC','Data Import',201], ['PRD','Regression',101]
    ].map(key => JSON.stringify(key)).sort();
    await expect.poll(() => page.locator('[data-matrix-row]').evaluateAll(elements => elements.map(e => e.getAttribute('data-matrix-row')).sort())).toEqual(expected);
    ...
    await expect(cell(page, 'TST', 'Data Import', 999).getByRole('button')).toHaveText('·');
    for (const catalogRootId of [1,20,99999]) {
        await server.seed({ ...matrixConfig, catalogRootId }); await page.reload(); await open(page);
        await expect.poll(() => page.locator('[data-matrix-row]').evaluateAll(elements => elements.map(e => e.getAttribute('data-matrix-row')).sort())).toEqual(expected);
    }
    expect(server.azure().writes).toEqual([]);
});

test('V4-H02 RM3-02 renames a selected version by ID and disambiguates equal titles with paths and IDs', async ({ page }) => {
    await open(page);
...
test('V4-H03 RM3-02 a selected version outside the active plan is explained and cannot write', async ({ page }) => {
...
for (const [id, name] of [[31, 'tst'], [31, 'TST erweitert'], [32, 'regression'], [32, 'Regression erweitert']] as const) {
    test(`V4-H04 RM3-03 RM3-04 exact complete names reject source ${id} renamed ${name}`, async ({ page }) => {
...
test('V4-H05 RM3-03 RM3-07 descendants at an extra level never stand in for either direct child', async ({ page }) => {
...
test('V4-H06 RM3-07 distinguishes missing environments, missing contents and absent direct membership', async ({ page }) => {
...
for (const duplicateEnvironment of [false, true]) {
    test(`V4-H07 RM3-07 RM3-09 resolves multiple direct paths only by concrete content selection (environment duplicate ${duplicateEnvironment})`, async ({ page }) => {
...
test('V4-H08 RM3-08 zero runs with valid points shows NotRun dash and permits a manual run', async ({ page }) => {
...
test('V4-H09 RM3-11 parentless flat catalog still resolves the full loaded tree', async ({ page }) => {
...
for (const failure of ['tree-error', 'incomplete-tree'] as const) {
    test(`V4-H10 RM3-11 ${failure} is a load failure with no writable cells`, async ({ page }) => {
...
test('V4-H11 RM3-11 failed tree refresh marks retained results stale and blocks writes until recovery', async ({ page }) => {
...
test('V4-H12 RM3-14 changing or removing all ancestor and concrete Suite tags changes no matrix source or Azure tags', async ({ page }) => {

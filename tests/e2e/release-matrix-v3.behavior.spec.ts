...
test('V3-B01 RM3-05 RM3-10 grouping changes only group headings and opposite row context', async ({ page }) => {
    await open(page);
    const rowKeys = await page.locator('[data-matrix-row]').evaluateAll(elements => elements.map(e => e.getAttribute('data-matrix-row')).sort());
    await expect(rows(page, 'TST', 'Regression', 201).getByRole('rowheader').locator('small')).toHaveText('Regression');
    ...
    await expect(rows(page, 'TST', 'Regression', 201).getByRole('rowheader').locator('small')).toHaveText('TST');
...
test('V3-B02 RM3-01 RM3-13 version selection, visibility and order persist by ID without editable title fields', async ({ page }) => {
    ...
    await expect(page.getByRole('table').getByRole('columnheader')).toHaveText(['Testfall','2.1.0','2.0.0']);
    ...
    await expect(page.getByRole('table').getByRole('columnheader')).toHaveText(['Testfall','2.1.0','2.0.0','2.2.0']);
});

test('V3-B03 RM3-09 a write targets one physical content point and preserves other versions, environments and contents', async ({ page }) => {
...
test('V3-B04 RM3-09 completed-run precedence and point fallback still resolve the same concrete point', async ({ page }) => {
...
test('V3-B05 RM3-09 compact known/custom outcomes and point-count guards remain unchanged', async ({ page }) => {
    await open(page);
...
test('V3-B06 RM3-09 RM3-13 keyboard cancellation, confirmation and all manual outcomes preserve the compact controls', async ({ page }) => {
...
test('V3-B07 RM3-09 errors before and after run creation remain visible without a false success', async ({ page }) => {
...
test('V3-B08 RM3-10 exact case-tag, direct-membership and search filters never change column sources', async ({ page }) => {
    ...
    await expect(page.locator('[data-matrix-row]')).toHaveCount(1); await expect(rows(page, 'TST', 'Regression', 303)).toBeVisible();
    await page.getByRole('button', { name: 'Filter zurücksetzen', exact: true }).click();
...
test('V3-B09 RM3-12 real v2 preferences migrate once, retaining catalog, filters, set and context while resetting tags', async ({ page }) => {
...
test('V3-B10 RM3-13 LowDB keeps each grouping order/collapse, filters and version state independently per set', async ({ page }) => {
...
test('V3-B11 RM3-13 equal environment and content names do not share collapse state', async ({ page }) => {
...
test('V3-B12 RM3-09 RM3-13 touch update followed by missing-membership tooltip stays within the viewport', async ({ browser }) => {
...
test('V3-B13 RM3-13 real horizontal and vertical scrolling preserve header, row context and tooltip bounds', async ({ page }) => {
    const columns=addVersionColumns(server.azure(),14);
    for(let id=400;id<460;id++){server.azure().membership[22].push(id);server.azure().titles[id]=`Zusatztest ${id}`;}
...
test('V3-B14 RM3-13 browser fallback survives unavailable preferences while a recovered LowDB remains authoritative', async ({ page }) => {
    await open(page);
    await page.getByLabel('Gruppieren nach',{exact:true}).selectOption('content');
    await page.getByLabel('Testfall suchen',{exact:true}).fill('201');
    await expect.poll(async()=>JSON.parse(await server.disk()).users.contract.setReleaseMatrices['matrix-set'].search).toBe('201');

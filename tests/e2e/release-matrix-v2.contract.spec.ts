...
const names:Record<number,string>={11:'Regression',12:'Data Import',13:'Allgemein'};
const rowSelector=(suite:number,id:number)=>`[data-matrix-row='${JSON.stringify([names[suite],id])}']`;
const row=(page:Page,suite:number,id:number)=>page.locator(rowSelector(suite,id)).first();
...
test('V2-C01 RM01 RM02 RM03 RM05 RM08 RM17 preserves logical groups and existing navigation',async({page})=>{
...
test('V2-C02 RM04 RM06 RM15 retains the editable ordered tag list and distinct sources',async({page})=>{
  ...
  await expect(page.locator("[data-matrix-row='[\"Regression\",201]']")).toHaveCount(2);await expect(page.locator("[data-matrix-row='[\"Data Import\",201]']")).toHaveCount(2);
...
test('V2-C03 RM09 RM10 RM13 distinguishes absence, NotRun, unknown outcomes and ambiguous write targets',async({page})=>{
  await open(page);
...
test('V2-C04 RM10 RM16 copies existing chips and exposes full outcome on hover and focus',async({page})=>{
...
test('V2-C05 RM11 RM12 RM14 writes a new isolated completed run, preserves history and confirms readback',async({page})=>{
...
test('V2-C06 RM11 RM14 offers completed outcomes without a destructive reset and keeps tag copies coherent',async({page})=>{
  ...
  await page.getByLabel('Gruppieren nach',{exact:true}).selectOption('tags');await expect(page.locator(rowSelector(11,201))).toHaveCount(2);await expect(page.locator(rowSelector(12,201))).toHaveCount(2);await outcome(page,11,201).selectOption('Blocked');
  await expect(page.getByRole('status').filter({hasText:'Durchlauf bestätigt'})).toBeVisible();
  await expect(page.locator("[data-matrix-row='[\"Regression\",201]'] [data-matrix-column=\"test\"] select")).toHaveCount(2);
  for(const element of await page.locator("[data-matrix-row='[\"Regression\",201]'] [data-matrix-column=\"test\"] select").all())await expect(element).toHaveValue('Blocked');
  await expect(page.locator("[data-matrix-row='[\"Data Import\",201]'] [data-matrix-column=\"test\"] select")).toHaveCount(2);
  for(const element of await page.locator("[data-matrix-row='[\"Data Import\",201]'] [data-matrix-column=\"test\"] select").all())await expect(element).toHaveValue('Passed');
});
test('V2-C07 RM12 RM14 separates write failure and a created run with unconfirmed result',async({page})=>{
...
test('V2-C08 RM18 RM19 combines actual suite membership, exact tags and title search without changing columns',async({page})=>{
...
test('V2-C09 RM06 RM07 RM15 persists catalog, release columns, mappings, group order and collapse per set',async({page})=>{
  ...
  await page.getByLabel('Spaltenname 3',{exact:true}).fill('Vergleich');await page.getByLabel('Suite-Tag 3',{exact:true}).fill('2.2.0-Test');await expect(page.getByLabel('Release-Wurzel 3',{exact:true})).toHaveCount(0);
...
test('V2-C10 RM20 requires explicit selection for duplicate names and preserves IDs through rename',async({page})=>{
  server.azure().suiteTags[43]=['2.1.0-Test'];server.azure().suiteTags[44]=['2.1.0-Test'];await server.seed({...matrixConfig,columns:[matrixConfig.columns[0]]});await page.reload();await open(page);await expect(cell(page,11,201).getByRole('button',{name:/Suite zuordnen/})).toBeVisible();await expect(outcome(page,11,201)).toHaveCount(0);
  ...
  server.azure().suiteTags[43]=[];await page.reload();await open(page);await expect(outcome(page,11,201)).toHaveCount(0);await expect(cell(page,11,201)).toContainText('?');
});
test('V2-C11 RM09 RM16 handles failed loads and empty configuration without fabricated statuses',async({page})=>{
...
test('V2-C12 RM03 RM06 RM08 reads the full catalog once per occurrence and retains existing point fallback',async({page})=>{
  ...
  await expect(page.locator("[data-matrix-row='[\"Regression\",101]']")).toHaveCount(1);await expect(row(page,13,303)).toBeVisible();await expect(outcome(page,11,201)).toHaveValue('Failed');
  expect(await page.locator("[data-matrix-row^='[\"Regression\",']").evaluateAll(rows=>rows.map(r=>r.getAttribute('data-matrix-row')))).toEqual([JSON.stringify(['Regression',201]),JSON.stringify(['Regression',101]),JSON.stringify(['Regression',103])]);
...
test('V2-C13 RM04 RM06 tag presentation preserves distinct group labels and has no effect on suite mode',async({page})=>{
...
test('V2-C14 RM09 RM10 RM13 explains noneditable targets and absent cells on hover, focus and tap',async({page})=>{
...
test('V2-C15 RM12 RM14 rejects failed confirmation and stale history without silently creating another run',async({page})=>{
...
test('V2-C16 RM01 RM12 RM14 locks identical copies and isolates a pending write across a set switch',async({page})=>{
  await open(page);await page.getByLabel('Gruppieren nach',{exact:true}).selectOption('tags');await expect(page.locator(rowSelector(11,201))).toHaveCount(2);server.azure().control.delayWrite=700;await outcome(page,11,201).selectOption('Passed');
  await expect(page.locator("[data-matrix-row='[\"Regression\",201]'] [data-matrix-column=\"test\"] select")).toHaveCount(2);
  for(const copy of await page.locator("[data-matrix-row='[\"Regression\",201]'] [data-matrix-column=\"test\"] select").all())await expect(copy).toBeDisabled();
...
test('V2-C18 RM18 RM19 combines exact case-insensitive filters, excludes descendants and clears back to catalog',async({page})=>{
...
test('V2-C19 RM20 exposes candidate paths, avoids fuzzy matching and detects removed explicit suites',async({page})=>{
  server.azure().suiteTags[43]=['2.1.0-Test'];server.azure().suiteTags[44]=['2.1.0-Test'];await server.seed({...matrixConfig,columns:[matrixConfig.columns[0]]});await page.reload();await open(page);await page.getByRole('button',{name:'Spalten & Gruppierung',exact:true}).click();const mapping=page.getByLabel('Suite für Regression / 2.1.0',{exact:true});expect(await mapping.locator('option').allTextContents()).toEqual(expect.arrayContaining([expect.stringMatching(/A.*Regression.*43/),expect.stringMatching(/B.*Regression.*44/)]));await mapping.selectOption('43');await expect(mapping).toHaveValue('43');
...
test('V2-C20 RM10 RM11 RM16 uses keyboard selection and exposes every compact status',async({page})=>{
...
test('V2-C21 RM09 RM16 touch targets and sticky matrix fit narrow and long screens',async({browser})=>{
...
test('V2-C22 RM16 pins row and column context while scrolling many releases and rows, and handles empty catalogs',async({page})=>{
  ...
  const firstX=(await first.boundingBox())!.x;await scroll.evaluate(el=>el.scrollLeft=300);expect(await scroll.evaluate(el=>el.scrollLeft)).toBeGreaterThan(0);expect(Math.abs((await first.boundingBox())!.x-firstX)).toBeLessThan(2);expect(await scroll.evaluate(el=>el.scrollHeight>el.clientHeight)).toBe(true);await scroll.evaluate(el=>{el.scrollTop=300;el.scrollLeft=0;});expect(await scroll.evaluate(el=>el.scrollTop)).toBeGreaterThan(0);const headBox=(await head.boundingBox())!;const scrollBox=(await scroll.boundingBox())!;expect(headBox.y).toBeGreaterThanOrEqual(scrollBox.y);expect(headBox.y).toBeLessThan(scrollBox.y+10);
